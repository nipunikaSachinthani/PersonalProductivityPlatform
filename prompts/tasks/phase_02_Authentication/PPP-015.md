...
> Protect API endpoints with JWT authentication middleware and implement role-based authorization policies.

This task transforms the API from open to secured. The JWT bearer middleware validates every incoming token — checking signature, expiry, issuer, and audience — and populates `HttpContext.User` with claims. Authorization policies (`[Authorize]`, `[Authorize(Roles = "Admin")]`) then gate access to specific endpoints. A rate-limiting middleware on auth endpoints (login, register, forgot-password) prevents brute-force attacks. This task teaches the ASP.NET Core authentication/authorization pipeline, middleware ordering, and defense-in-depth security patterns.
...
- JWT middleware in `Program.cs`: `AddAuthentication(JwtBearerDefaults.AuthenticationScheme).AddJwtBearer(...)`
- Token validation parameters: validate issuer, audience, lifetime, signature key
- `AddAuthorization()` with policies: "AdminOnly" → `RequireRole("Admin")`
- Apply `[Authorize]` globally or per-controller; `[AllowAnonymous]` on auth endpoints
- Rate limiting: `AddRateLimiter()` with fixed-window policy (e.g., 5 requests per minute) on `/api/v1/auth/*` endpoints
- Custom `Unauthorized` response: return `ApiResponse` with 401 and structured error
- Add `app.UseAuthentication()` before `app.UseAuthorization()` in middleware pipeline
...
1. **Branch:** `feature/PPP-015-protect-apis` off `main` (see GIT_WORKFLOW.md §3)
2. **Commits:** Conventional format — `feat(auth): add JWT auth middleware and RBAC policies` (see GIT_WORKFLOW.md §5)
...
```bash
# Try accessing a protected endpoint without token → 401
