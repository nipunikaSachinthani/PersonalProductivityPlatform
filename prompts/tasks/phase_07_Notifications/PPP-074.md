...
> Implement GetNotifications query (paginated, unread-first sort) and MarkAsRead/MarkAllAsRead commands with API endpoints.

The notification API serves the notification bell in the app header. GetNotifications returns the user's notifications sorted by unread first, then by creation date descending, with pagination. Each notification includes the type, title, message, read status, and a link to the related entity. MarkAsRead marks a single notification as read. MarkAllAsRead bulk-updates all unread notifications for the user. This task teaches paginated list APIs for temporal data, bulk update patterns, and read-status management.
...
- Query: `GetNotificationsQuery` with `Page`, `PageSize`, `UnreadOnly? (bool)`
- Sort: unread first (`IsRead ASC`), then `CreatedAt DESC`
- Commands: `MarkNotificationAsReadCommand` (NotificationId), `MarkAllNotificationsAsReadCommand`
- MarkAsRead handler: load notification, verify ownership, set `IsRead = true`, save
- MarkAllAsRead handler: execute `UPDATE Notifications SET IsRead = 1 WHERE UserId = currentUserId AND IsRead = 0`
- Also add unread count to response: include `unreadCount` in meta or as a separate header
...
1. **Branch:** `feature/PPP-074-notification-api` off `main` (see GIT_WORKFLOW.md §3)
2. **Commits:** Conventional format — `feat(notifications): implement notification query and read endpoints` (see GIT_WORKFLOW.md §5)
...
curl -X PUT http://localhost:5000/api/v1/notifications/1/read -H "Authorization: Bearer <token>"
curl -X PUT http://localhost:5000/api/v1/notifications/read-all -H "Authorization: Bearer <token>"
```
