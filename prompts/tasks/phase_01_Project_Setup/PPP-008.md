...
> Write a light integration test for the health check endpoint using WebApplicationFactory.

This task introduces the testing discipline that runs through all 12 phases. Using `WebApplicationFactory<T>` (ASP.NET Core's in-memory test server), the test boots the full API pipeline — middleware, DI, routing, DbContext — and asserts the health endpoint returns a 200 OK with the expected JSON structure. This is the first test in the test pyramid and establishes the pattern for all future integration tests. It also validates that the entire Phase 01 infrastructure works together correctly from a programmatic standpoint.
...
- Test project: `ProductivityPlatform.IntegrationTests` under `backend/tests/`
- Use `Microsoft.AspNetCore.Mvc.Testing` for `WebApplicationFactory<Program>`
- xUnit as the test framework
- Test class: `HealthCheckTests` with a single `[Fact]`
- Assert: status code 200, JSON contains `"status": "Healthy"`, `"database": "Connected"`
- Use `InternalsVisibleTo` in Api project or a public `Program` partial class for test access
- Consider using `CustomWebApplicationFactory` that overrides connection string for test isolation
...
1. **Branch:** `feature/PPP-008-health-check-test` off `main` (see GIT_WORKFLOW.md §3)
2. **Commits:** Conventional format — `test(setup): add integration test for health endpoint` (see GIT_WORKFLOW.md §5)
3. **PR:** Description with what was done, how tested, and a test plan (see GIT_WORKFLOW.md §6)
4. **Tests:** 1 integration test as described
