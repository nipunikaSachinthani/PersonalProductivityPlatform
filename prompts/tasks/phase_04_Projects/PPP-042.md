...
> Implement GetProjects list and GetProjectById queries with member data and membership-based authorization.

GetProjects returns all projects where the current user is a member (any role). Each project includes member summaries and a task count per status. GetProjectById returns a single project with full member details and — critically — checks that the requesting user is a member before returning data (non-members get 404). This task introduces resource-level authorization based on membership rather than simple ownership, a pattern that becomes the backbone of collaborative features.
...
- Query: `GetProjectsQuery` with optional `includeArchived (default false)` — filter by `ProjectMembers.Any(pm => pm.UserId == currentUserId)` and `IsArchived == includeArchived`
- Query: `GetProjectByIdQuery` with `Id` — eager load Members with User names, verify membership; non-member → `Result.NotFound()`
- Include member info: `ProjectDto.Members` contains `MemberDto { UserId, UserName (FirstName + LastName), Role }`
- Include task counts per status using `.Select()` projection or separate count query
- Endpoints: `GET /api/v1/projects`, `GET /api/v1/projects/{id}`
- Response: `ApiResponse<List<ProjectDto>>`, `ApiResponse<ProjectDto>`

**Concepts practiced in this task:**
- Membership-based authorization
- Eager loading related data (Include/ThenInclude)
- Aggregate data in DTOs (task counts)
...
1. **Branch:** `feature/PPP-042-get-projects` off `main` (see GIT_WORKFLOW.md §3)
2. **Commits:** Conventional format — `feat(projects): implement project list and detail queries` (see GIT_WORKFLOW.md §5)
